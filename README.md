...
- ⚠️ **QR Code 點名** — 三種點名裡最硬的一塊，我們也**已竭盡全力**。QR 的 `data` token 是伺服器端用一把金鑰對「時間」簽出來的，經過整輪逆向，**我們目前尚未發現學生端能偽造或憑空取得它的方法**（但**已知有人實際做到、原理我方尚未掌握，所以是「目前未解」、不是「不可能」**；完整研究與踩過的坑見下方〈QR 資料 Token 逆向研究紀錄〉）。目前唯一能自動化的方式，是利用實測發現的「**`data` 跨課可攜**」特性，搭配一個你自備、能發起 QR 點名的教師帳號即時取得當下 `data` 再送出（「教師輔助」）——這是**暫時的妥協**，離真正「免教師自助簽到」的目標還有距離，我們仍在找路。
...
關於 QR（誠實現況）：QR 的 `data` token 純在伺服器端以「金鑰 × 時間」簽發、學生端 API 不會給，而且我們已徹底逆向、**目前尚未發現任何從外部偽造或取得的方法**（**已知有人實際做到、原理我方未掌握，故非「不可能」，只是「目前未解」**；細節與所有踩過的坑見下方〈QR 資料 Token 逆向研究紀錄〉）。因此在找到真正的自助解法之前，自動化只能靠「**教師輔助**」這個暫時妥協——它建立在實測發現的「`data` 跨課可攜」特性上：用你自備的教師帳號即時發起一場 QR 點名、讀出當下 `data`，再用學生帳號送出；教師登入失敗只會停用 QR 教師輔助，不影響數字 / 雷達點名。真的沒有教師帳號時，就只能手動貼上 / 掃描當下的 QR 內容——這確實離「免教師自助簽到」的目標還很遠，是目前的誠實現況，我們仍在找路。
...
### 監控永不卡住：一偵測到就「分派」下去、迴圈繼續跑（v1.8-alpha.3）

監控迴圈本身只做三件事：**輪詢 → 分派 → 去重**。每一輪它問伺服器「現在有沒有點名／有沒有待答活動」，一偵測到就把「這件事的完整處理」丟成一個背景工作（等 15% 門檻、送出、回查、關閉收尾都在背景跑），迴圈自己**立刻**回去輪詢下一輪——所以慢動作的處理（例如等門檻、或 LLM 想答案想很久）再也不會把整個監控拖住卡死。同一場點名／同一個活動只要已經在處理中，就不會被重複分派。

這也讓自動答題變成**真正並行**：一偵測到活動就馬上顯示（不必等 LLM 想完），LLM 備答與 15 秒緩衝倒數同時進行，多個活動的多個倒數也各自獨立；投票若你早就投過了，直接跳過不再重試。
...
未設定教師帳號時，程式保留兩條**手動**路徑：`tron qr paste`（貼上 QR 內容）與 `tron qr scan`（本機掃描器／從圖片解碼，需另裝 `qr-image` 套件）。（v1.8 起移除了會偷讀剪貼簿、卻假裝成「自動」的背景剪貼簿送出——不誠實，只留明確的手動路徑。）
...
> ⚠️ 這節是**研究筆記**，不是攻擊教學。結論先講：**我們目前（從外部）尚未發現任何偽造或憑空生出 QR `data` token 的方法**——但**已知有人實際做到**、只是原理我方尚未掌握，所以這是「**目前未解**」、**不是「不可能」**。
...
- ⇒ **我們從外部尚未找到偽造法**（不代表不可能——已知確實有人做到）。能做到的人，手上很可能**已握有那把伺服器金鑰**（來自我們碰不到的管道）。
...
- 但若金鑰是 **128 位隨機**（WisdomGarden 的風格——挖到的那把直播錄影金鑰就是 32 位隨機 hex）：要試 **2¹²⁸**，一張卡約 **5×10¹⁹ 年**（≈ 宇宙年齡的 35 億倍）→ **暴力搜金鑰這條路 physically 打不動**（跟爆 AES-128 / 比特幣私鑰同級）。⚠️ 注意這只是說「**盲目暴力爆破**」行不通,**不代表整件事不可能**——前輩已實際做到,顯然走的不是暴力這條。
...
**更新（2026-07）**：用廠商自家的 `orgs.json` 對 **875 個租戶**做了版本普查（各校 TronClass 版本不一，想找還在跑舊版的活站）——結果全網**最舊的活實例只到 1.62**，沒有任何 2015–2018 的古早版還活著（都在託管雲、被自動更新）。再派多個 agent 深掘最舊那幾站（含**舊版 AngularJS 的 rollcall 模組**，與 2020 手機 App 同世代）：它們**也全是 consume-only**——只把伺服器回傳的字串畫成 QR 圖、沒有生成碼、沒有內嵌金鑰；上萬個榨出的候選金鑰對語料全 NO MATCH。⇒ 就我們摸得到的 client 而言，**QR 的 `data` 很可能一直是伺服器端生成、這些 client 從沒拿過那把金鑰**——連最早的 AngularJS 時代都是。所以「舊 client 內嵌金鑰」**這一條路**看來很渺茫；但這只代表**這條路**渺茫，**不代表整件事沒戲**——前輩顯然另有辦法（很可能走的是「握有伺服器金鑰」那條，只是我方尚未掌握）。
...
**更新（2026-07，「說不定金鑰早在手上、只是演算法試錯」——也驗遍了）**：把 App 與廠商程式碼裡**所有硬編碼祕密**（LRP 簽章金鑰、TalkingData／Bmob／OneSignal 等 32-hex 常數、各 DB／OAuth 密碼…）整成一份**有出處的主金鑰清單**，再用一支可重用掃描器對**活 token** 逐一比對——不只 MD5/SHA/HMAC，還加了 **AES-128/192/256 加密時間**（「不是雜湊、是加密」的假設）、AES-CMAC、keyed-BLAKE2、以及 App 實際用的 `md5(前綴+時間+祕密)` 樣式——**共 95,859 種構造、全 NO MATCH**。⇒ **金鑰不在任何可達祕密裡**，再次印證它 server-only。好處：這份清單＋掃描器留著，日後若冒出新金鑰或新演算法，秒級可重驗。
...
> **研究起點**：這場調查源於一個說法——**有人聲稱「純學生端」就能偽造或取得當下的 `data`，免教師、跨校通用、涉及密碼學、連工程師都猜不到**。我們據此把每一條密碼學／端點／client／協定的路都窮盡驗證了一遍（見上方負面結果地圖），**到目前為止，以外部可達的一切都尚未能重現**（**不是不可能——他確實做到了，只是原理我方尚未掌握**）。不假裝成功，把地圖留給後人。
