...
> 🔢 **輔仁 (FJU) 的登入頁有 4 碼數字圖形驗證碼**：程式用本地離線 OCR 自動辨識。**打包版（exe）已內建 OCR**，首次使用時自動下載辨識模型（約 14MB）；原始碼安裝請加裝 `pip install -e .[ocr]`。取得不到模型時自動退回「手動瀏覽器登入」一次後沿用 cookie，其餘點名功能完全相同。
...
- **填學校代號 → 自動登入**：`THU` / `TKU` / `TRONCLASS` / `SCU` / `FJU`（東吳請填 `SCU`，別填 `TRONCLASS`）。這時程式用你填的 `user` + `passwd` 自動登入，全程零操作。也接受中文別名（例如 `帳號 = s1234567`、`密碼 = mypassword`、`學校 = 東海` 或 `學校 = 輔仁`）。輔仁 (FJU) 的登入驗證碼由內建 OCR 自動辨識（打包版內建、首次自動下載模型；原始碼安裝用 `pip install -e .[ocr]`）。
