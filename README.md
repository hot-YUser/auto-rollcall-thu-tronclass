...
> **exam（線上測驗）、questionnaire（問卷）、homework（作業）** 已端到端實機驗證（真實出題→真實作答→確認送出）；
> **classroom_exam（即時測驗）** 的建題／開放已實機驗證，學生送出受伺服器端「即時測驗現場連線狀態」把關（單純 REST 無法重現該即時狀態）；
> **vote（投票）** 的建立已實機驗證，學生投票送出的 request body 無法從還原碼確認、實測回 500，列為待補；
> **courseware_quiz（教材測驗）** 因測試租戶無 AI 額度無法建題，端點為逆出假設。
> 後三者的學生端最終送出留待 v1.7-alpha.2 補強。可在 `types` 移除你尚未需要的題型。
