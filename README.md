...
- Teacher：`teacher status` 會登入後自動判斷目前帳號是 `teacher`、`student`、`mixed` 或 `unknown`；`teacher course` / `teacher rollcalls` / `teacher enrollments` / `teacher groups` / `teacher group-set` / `teacher bulletins` / `teacher org-bulletins` / `teacher students` / `teacher activities` / `teacher scores` / `teacher activity` / `teacher exam` / `teacher classroom` / `teacher questionnaire` / `teacher course-estimates` / `teacher course-packages` / `teacher rollcall-students` 會讀取教師端課程結構、分組、課程公告、校級公告、名單、點名、作業、測驗、隨堂測驗、問卷、課程評價、課程包、討論、活動明細/刪除檢查、rubric 與成績摘要。另有 `teacher platform`、`teacher catalog`、`teacher media`、`teacher authoring`、`teacher completion-criteria`、`teacher course-completion-criteria`、`teacher forum-categories`、`teacher forum-category`、`teacher activity-uploads-license`、`teacher subject-libs`、`teacher subject-lib-subjects`、`teacher subject-lib-statistic`、`teacher subject-lib-knowledge-nodes`、`teacher subject-lib-folders`、`teacher questionnaire-submissions`、`teacher course-estimate-replies`、`teacher course-estimate-user`、`teacher course-package-course`、`teacher courseware-quizzes`、`teacher courseware-quiz-subjects`、`teacher courseware-quiz-settings` 讀取前端已觀察到的組織/學期/課程計畫、課程/使用者/班級 catalog、媒體/直播/會議/第三方整合、課程製作/project/blueprint、outline setting、feedback activity、H5 courseware、批改附件、完成條件、討論分類、資源授權、題庫、問卷作答、課程評價、課程包與互動教材測驗資料。教師端寫入已有 rollcall、score、rubric、bulletin、module/syllabus、group/group-set、data-import、activity/resource/comment、homework、submission、exam、classroom、forum、subject-lib、questionnaire、course-estimate、course-package、courseware-quiz、course sync/enrollment、outline/outline-setting、Chinamcloud 類專用命令，例如 `teacher update-score-publish-item-maps`、`teacher submit-edu-scores`、`teacher create-rubric`、`teacher create-bulletin`、`teacher create-module`、`teacher sort-syllabuses`、`teacher resort-activity`、`teacher create-activity`、`teacher publish-activities`、`teacher save-activity-resource`、`teacher log-activity-read`、`teacher update-activity-resource`、`teacher add-activity-comment`、`teacher operate-activity-comments`、`teacher import-enrollments`、`teacher import-scores`、`teacher import-seat-numbers`、`teacher create-group-set`、`teacher create-group`、`teacher update-group-members`、`teacher check-activity-dependents`、`teacher sync-courses-from-urp`、`teacher update-enrollment-role`、`teacher update-enrollments-role`、`teacher delete-enrollment`、`teacher delete-enrollments`、`teacher update-course-outline`、`teacher notify-outline-editing`、`teacher create-outline-setting`、`teacher update-outline-setting`、`teacher sort-outline-setting`、`teacher delete-outline-setting-option`、`teacher toggle-outline-setting`、`teacher update-outline-required-options`、`teacher update-chinamcloud-resources`、`teacher update-forum-status`、`teacher create-subject-lib`、`teacher copy-subject-lib`、`teacher update-subject-lib`、`teacher move-subject-libs`、`teacher copy-subject-libs-to-user`、`teacher move-subject-lib-subjects`、`teacher copy-subject-lib-subjects`、`teacher delete-subject-lib-subjects`、`teacher create-questionnaire-subject`、`teacher import-questionnaire-subjects`、`teacher create-course-estimate`、`teacher create-course-estimate-reply`、`teacher create-course-package`、`teacher export-course-package`、`teacher update-course-package`、`teacher import-course-package`、`teacher create-courseware-quiz-subjects`、`teacher update-courseware-quiz-subjects`、`teacher generate-courseware-quiz-subjects`、`teacher format-courseware-quiz-question`、`teacher copy-subject-libs-to-courseware-quiz`、`teacher recommend-submissions`、`teacher delete-exams`、`teacher create-classroom-exam`、`teacher save-classroom-subjects`、`teacher score-classroom`。`teacher get <api-path>` 可用同一套登入與遮蔽規則讀取尚未包成專用指令的教師端 API；`teacher request <method> <api-path>` 可 dry-run 預覽尚未包裝的教師端寫入。預設會遮蔽 email、學號、點名碼、座標等敏感欄位；只有明確加 `--include-sensitive` 才會顯示教師端敏感值。會修改教師端資料的教師命令與 `teacher request` 預設只做 dry-run，需要同時加 `--execute --yes` 才會真的送出。
- Teacher rollcall API coverage：新增點名專用命令覆蓋建立、模組建立、啟動、心跳、QR/數字/雷達/自助點名停止、刪除、學生 QR/數字/雷達送出、批次更新單次名單、更新單一學生歷史點名、學生名單/歷史點名/請假紀錄/進行中點名/單次明細/分頁明細/統計/result 查詢，以及 `teacher qrcode --url ... --output ...`。其中雷達簽到採 APK 確認的 `/api/rollcall/{id}/answer?api_version=1.76`；所有寫入命令仍預設 dry-run，需要 `--execute --yes`。
- Teacher resource library：新增 `teacher resource-groups`、`teacher resource-group-resources`、`teacher user-resources`、`teacher shared-resources`、`teacher shared-resource-comments`、`teacher cc-license-groups`、`teacher entries`、`teacher slides` 等前端觀察到的資源庫/共享資源/條目/投影片讀取命令；並補 `teacher create-resource-group`、`teacher update-resource-group-resource`、`teacher save-shared-resource`、`teacher publish-shared-resource`、`teacher create-entry`、`teacher update-slide`、`teacher export-slide` 等 JSON 寫入包裝，全部同樣預設 dry-run。
- Teacher downloads：新增 `teacher download`、`teacher download-upload`、`teacher download-upload-thumbnail`、`teacher download-upload-modified-image`、`teacher download-upload-swf`、`teacher download-upload-reference`、`teacher download-shared-resource`、`teacher download-wedrive-file`、`teacher download-third-part-upload`，可在登入後下載前端觀察到的教師端檔案/縮圖/共享資源/WeDrive/第三方預覽二進位內容；預設不覆蓋已存在檔案，需加 `--overwrite`。
- Teacher exports：新增 `teacher download-request` 泛用二進位 GET/POST 下載，以及 `teacher export-questionnaire`、`teacher export-topic`、`teacher export-category-topics`、`teacher export-shared-resource-subject-lib`、`teacher export-shared-resource-stat`、`teacher export-shared-resource-video-stat`、`teacher export-cloud-classroom-live-classes`、`teacher export-tencent-meeting-statistics`，可取回前端觀察到的 Excel/arraybuffer 匯出內容；POST 匯出預設 dry-run，需要 `--execute --yes`。
- Teacher analytics / AIR credit / calendar / management：新增 `teacher stats`、`teacher air-credit`、`teacher calendar-meetings`、`teacher management-calendar-meetings`、`teacher teaching-calendars`、`teacher vtrses`、`teacher departments`、`teacher ai-ppt`、`teacher platform`、`teacher org-bulletins`、`teacher catalog`、`teacher media`、`teacher authoring`，包裝前端觀察到的課程統計/TPDOE 學生統計、AIR credit 狀態/使用者 token/使用者列表/課程列表/稽核/授課學期課程/資源、日曆會議、管理會議、教學行事曆、VTRS 虛擬教室/access-code、部門出席、AI PPT 用量、組織/學年/學期/課程計畫/虛擬教室資源/OBE/todos/jobs/inclass-report/sign-in stats/最近造訪課程/alert/calendar event/timetable/instruction-team meeting、校級公告、課程/使用者/班級/課綱 catalog、合併課程、課程互動/投票/互動提交、campus subject-lib、學生預警、資料匯入入口、影音資源、WeDrive、Lark、騰訊會議、lecture-live、ClassIn、釘釘直播、互動活動、課程範本、knowledge node、project/blueprint、題目、feedback activity、danmu、upload details/document preview URL/knowledge graph/knowledge base、my-notes、correction-books、authz courses、portal-logo、H5 courseware 與批改附件查詢；另有 `teacher export-stat-students`、`teacher export-stat-report`、`teacher export-stat-courses`、`teacher export-stat-attendance`、`teacher export-department-user-attendance`、`teacher export-department-attendance`、`teacher export-stat-vtrses-data`、`teacher export-ai-ppt-usage`、`teacher export-air-credit`、`teacher export-management-calendar` 下載統計/出席/VTRS/AI PPT/AI 點數/會議 Excel 或 CSV。日曆會議、教學行事曆與 AIR credit 管理寫入已有 `teacher create-calendar-meeting`、`teacher update-calendar-meeting`、`teacher delete-calendar-meeting`、`teacher create-teaching-calendar`、`teacher update-teaching-calendar`、`teacher delete-teaching-calendar`、`teacher create-air-credit-assignments`、`teacher update-air-credit-assignments`、`teacher update-air-credit-status`、`teacher clear-air-credit-remaining`、`teacher update-air-credit-course-usage-limit`，全部預設 dry-run，需要 `--execute --yes`。
- Teacher latest coverage：課程報表另讀取 activity reads、學生閱讀狀態、課程 members 與 certification；`teacher platform` 另涵蓋 authz permissions、course permissions、my classes、my teaching classes 與 task-last；`teacher catalog` 另涵蓋課程數量、公開課程、certifications、management certifications 與 course subjects；`teacher media --upload-id` 可探測 upload preview/audio metadata，並涵蓋 public resources；`teacher authoring` 另涵蓋 projects list。
- Teacher dump gap closure：`teacher stats` 另涵蓋 courses stats 與 activities-for-courses；`teacher platform` 另涵蓋 portal classifications、authz user roles、in-progress homeworks、alert logs、org change plans、third-party info、latest topics 與 user-index/profile stats；`teacher catalog` 另涵蓋 users without authz roles、course cover list、course shared records、course certification 與 campus subject-lib classification counts；`teacher media` 另涵蓋 caption task progress、ShanghaiTech library resources 與 video-suite comments；`teacher authoring` 另涵蓋 custom knowledge graph stat 與 KFS subjects。
- Teacher statistics/status：`teacher stats` 另涵蓋 course settings、homework submission status、exam submissions、score/zip status、stat bulletins/materials/video/scorm/weblink/org/semester/user-info、VTRS enable/count info 與 user course-certification scores；`teacher platform` 另涵蓋 org lang settings 與 alert members；剩餘需要具體 id 的 shared-resource blob、third-party preview 等二進位路徑仍可用既有專用下載命令或 `teacher download-request` 指向原始 API path 下載。
- Teacher uploads：新增 `teacher upload-file <path>`，依前端觀察到的 `/api/uploads` pre-upload + storage upload 流程送出檔案，支援 local/S3/Qiniu 形狀；預設 dry-run，只在同時加 `--execute --yes` 時上傳。
...
```bash
python -m troTHU.tron teacher create-rollcall <course-id> --payload-json "{...}" --execute --yes
python -m troTHU.tron teacher create-module-rollcall <course-id> --payload-json "{...}" --execute --yes
python -m troTHU.tron teacher start-rollcall <rollcall-id> --execute --yes
python -m troTHU.tron teacher activate-rollcall <rollcall-id> --execute --yes
python -m troTHU.tron teacher stop-qr-rollcall <rollcall-id> --execute --yes
python -m troTHU.tron teacher update-rollcall <rollcall-id> --payload-json "{...}" --execute --yes
python -m troTHU.tron teacher rollcall-info <rollcall-id> --json
python -m troTHU.tron teacher rollcall-students-page <rollcall-id> --page 1 --page-size 20 --json
python -m troTHU.tron teacher student-rollcalls <student-id> --course-id <course-id> --json
python -m troTHU.tron teacher qrcode --url "text-or-url" --output teacher-downloads/qrcode.png
```
