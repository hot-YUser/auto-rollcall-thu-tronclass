...
**那還剩什麼？** 誠實說，**從外部能做的都做盡了**；真正還有機會、卻都不在我們手上的只剩三條：(1) **握有該方法／金鑰的人**給出任何片段（一行碼、函式名、端點、或金鑰本身）——黑箱重建不出一把設計正確的 128 位金鑰；(2) **內部／內線／歷史伺服器洩漏**這種外部碰不到的後端缺口；(3) **一個還沒想到的全新攻擊面**（見下方「其他可能的面」）。
