...
try:  # secret redaction when reading records back (research-mode records are raw on disk)
...
def read_jsonl_records(path: Path, limit: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    for line in lines:
        ...
        try:
            ...
            record = {"raw": line}
        # research-mode records are written raw; re-redact on read so tail/summarize/export
        # never surface secrets even when the on-disk file is unredacted.
...
def summarize_logs(log_dir: Path, max_files: int = 100) -> Dict[str, Any]:
    ...
    for path in files:
        for record in read_jsonl_records(path):
            ...
            level_counts[str(record.get("level") or "INFO").upper()] += 1
    return {
        "log_dir": str(log_dir),
