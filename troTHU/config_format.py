...
_ADVANCED_COMMENTS = {
    ...
    "number.concurrency": "暴力猜碼時的並發請求數（已棄用，實際固定為 1，unknown-safe 單飛循序）",
    ...
    "qr_remote": "遠端 QR data 來源：從自架 data oracle 取當下 data；無教師來源或教師來源失敗時使用",
    "qr_remote.enabled": "true = 啟用；偵測到 QR 點名且本機教師來源不可用時，改從下面的服務取 data 送出",
    ...
    "number.min_concurrency": "暴力猜碼時的最低並發數（已棄用，實際固定為 1，unknown-safe 單飛循序）",
