...
async def number(main_session: ctx.aiohttp.ClientSession, rcid: int, *, my_user_no: str = "", endpoints: ctx.Any = None, request_ssl: ctx.Any = None) -> str:
    ...
    # Unknown-safe invariant: at most one unresolved Number mutation per
    # rollcall/profile. Retained knobs (concurrency/min_concurrency) are kept for
    # backward compatibility but clamped to effective 1.
    effective_concurrency = 1  # noqa: F841 - documents unknown-safe single-flight; retained knobs are clamped
    ...
    _ = effective_concurrency  # keep linter quiet while preserving the invariant name
    if current_concurrency != 1 or configured_concurrency != 1 or min_concurrency != 1:
        ctx.log_print(
            "Number concurrency configured={} clamped to effective=1 (unknown-safe sequential; at most one unresolved submit per rollcall/profile).".format(
                ...
                async with session.put(request_url, json=payload) as resp:
                    ...
                    if classification.status == ctx.NumberAttemptStatus.SUCCESS:
                        ...
                        if not (verification.get("ok") and verification.get("status") == "on_call_fine"):
                            ...
                            await ctx.mes(
                                "Number code {} submitted but on_call_fine not yet confirmed; will recheck next round.".format(
                                    submitted_unconfirmed_code
                                )
                            )
                        ...
                        await ctx.mes("Number rollcall succeeded!", highlight_block=banner)
                        return "success"
                        ...
                        if not submitted_unconfirmed_code:
                            submitted_unconfirmed_code = payload["numberCode"]
                        await ctx.mes(
                            "Number code {} submitted (ambiguous response), awaiting on_call_fine confirmation.".format(
                                submitted_unconfirmed_code
                            )
                        )
                        return "submitted_unconfirmed"
                        ...
                        if fatal_error is None and found_code == "NA":
                            fatal_error = ctx.UnauthorizedError(classification.message or "Login expired during number rollcall.")
                            stop_event.set()
                        return "fatal"
                    elif classification.status == ctx.NumberAttemptStatus.TRANSIENT_FAILURE:
                        last_transient_error = ctx.UnexpectedResponseError(
                            "HTTP {}: {}".format(resp.status, classification.message or body[:200])
                        )
                        return "transient"
                    else:
                        if fatal_error is None and found_code == "NA":
                            fatal_error = ctx.UnexpectedResponseError(
                                "HTTP {}: {}".format(resp.status, classification.message or body[:200])
                            )
                            stop_event.set()
                        return "fatal"
    ...
    async with ctx.aiohttp.ClientSession(**session_kwargs) as session:
        ...
        try:
            ...
            while next_code < ctx.NUMBER_CODE_LIMIT and (not stop_event.is_set()):
                result = await try_number_code(session, next_code, method="brute_force")
                if result == "wrong":
                    cooldown_tracker.reset()
                    next_code += 1
                    continue
                if result == "transient":
                    decision = cooldown_tracker.record_attempt(True)
                    if not decision.should_cooldown:
                        continue
                    if decision.exhausted:
                        fatal_error = last_transient_error or ctx.UnexpectedResponseError(
                            "Too many transient number errors, giving up."
                        )
                        stop_event.set()
                        break
                    ctx.status_print(
                        "Number rollcall throttled/server error, cooling down {:.1f}s before retry (effective concurrency 1 sequential)".format(
                            cooldown_seconds
                        )
                    )
                    ...
                    continue
                cooldown_tracker.reset()
                if result in ("success", "submitted_unconfirmed", "fatal", "stopped"):
                    break
                next_code += 1
        finally:
            progress_done.set()
    ...
    text = "Total time: {:.2f}s\nTotal request: {}/{}{}\nCode: {}\n".format(
        elapsed, request_count, ctx.NUMBER_CODE_LIMIT, " (Stopped early)" if found_code != "NA" else "", found_code
    )
