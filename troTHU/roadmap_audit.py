...
_COMPETITORS: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "absorbed": [
            "course/semester discovery facade, research sandbox gate, browser availability check, shape-only risky endpoint probe, local app shell contracts, and logs UX",
        ],
        "remaining_gap": [
            "full GUI is not implemented",
            "browser automation remains optional opt-in login/research support",
            "direct answer lookup stays excluded from daily flow unless a separate live probe proves THU behavior",
        ],
    ...
    },
    {
        ...
        "absorbed": [
            "QR diagnostic parser, compact payload variants, query-only inputs, alias handling, unknown field preservation, static image decode entrypoint, and synthetic fixture corpus",
        ],
        "remaining_gap": [
            "image decoding depends on optional decoder packages",
...
_CAPABILITIES: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "recommended_next_step": "include QR and static image decode in R1 manual validation; keep App camera work optional.",
        "estimated_rounds": 0,
    ...
    },
    {
        ...
        "gap": "user-facing troubleshooting flow can be clearer; opt-in network probe now covers QPS-style connection checks.",
    ...
    },
    {
        ...
        "current_state": "research sandbox, safe target allowlist, browser metadata check, provider fixture validation, and shape-only student_rollcalls probe gate are implemented.",
        "gap": "direct answer lookup is still locked pending live THU probe evidence and must never feed daily automation.",
        "recommended_next_step": "run the gated probe only during explicit research, then record whether THU exposes the same shape.",
        "estimated_rounds": 0,
        "blocks_upper_replacement": False,
