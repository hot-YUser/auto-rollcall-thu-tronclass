...
class ProgressBar {
  ...
  set percent(val) {
    ...
    _classPrivateFieldGet(this, _style).setProperty("width", `${_classPrivateFieldGet(this, _percent)}%`);
  }
  setWidth(viewer) {
    if (!viewer) {
      return;
  ...
  load(pdfDocument) {
    ...
    firstPagePromise.then(pdfPage => {
      ...
      Promise.all([_ui_utils.animationStarted, storedPromise, pageLayoutPromise, pageModePromise, openActionPromise]).then(async _ref3 => {
        ...
        const locationParams = (0, _ui_utils.parseQueryString)(document.location.search.substring(1));
        const filePage = locationParams.has('page') ? locationParams.get('page') : stored.page;
        if(filePage) {
          hash = hash ? `${hash}&page=${filePage}` : `page=${filePage}`
        };
        let rotation = null;
        ...
        if (stored.page && viewOnLoad !== ViewOnLoad.INITIAL) {
          hash = `page=${filePage}&zoom=${zoom || stored.zoom},` + `${stored.scrollLeft},${stored.scrollTop}`;
