...
class FontProvider {
    ...
    static FontsContractCompat.FontInfo[] query(Context context, FontRequest fontRequest, String str, CancellationSignal cancellationSignal) throws Throwable {
        ...
        Cursor cursor = null;
        try {
            Cursor cursorQuery = Api16Impl.query(context.getContentResolver(), uriBuild, new String[]{"_id", FontsContractCompat.Columns.FILE_ID, FontsContractCompat.Columns.TTC_INDEX, FontsContractCompat.Columns.VARIATION_SETTINGS, FontsContractCompat.Columns.WEIGHT, FontsContractCompat.Columns.ITALIC, FontsContractCompat.Columns.RESULT_CODE}, "query = ?", new String[]{fontRequest.getQuery()}, null, cancellationSignal);
            if (cursorQuery != null) {
                try {
                    if (cursorQuery.getCount() > 0) {
                        ...
                        while (cursorQuery.moveToNext()) {
                            ...
                            arrayList.add(FontsContractCompat.FontInfo.create(uriWithAppendedId, i2, columnIndex5 != -1 ? cursorQuery.getInt(columnIndex5) : Videoio.CAP_PROP_XI_DOWNSAMPLING, columnIndex6 != -1 && cursorQuery.getInt(columnIndex6) == 1, i));
                        }
                    }
                } catch (Throwable th) {
                    th = th;
