...
final class PathRelativizer {
    ...
    public final Path tryRelativeTo(Path path, Path base) {
        ...
        Path pathRelativize = pathNormalize.relativize(r);
        int iMin = Math.min(pathNormalize.getNameCount(), r.getNameCount());
        for (int i = 0; i < iMin; i++) {
            ...
            if (!Intrinsics.areEqual(r.getName(i), path2)) {
                throw new IllegalArgumentException("Unable to compute relative path");
            }
        }
        if (Intrinsics.areEqual(r, pathNormalize) || !Intrinsics.areEqual(pathNormalize, emptyPath)) {
            ...
            r = StringsKt.endsWith$default(string, separator, false, 2, (Object) null) ? pathRelativize.getFileSystem().getPath(StringsKt.dropLast(string, pathRelativize.getFileSystem().getSeparator().length()), new String[0]) : pathRelativize;
        }
