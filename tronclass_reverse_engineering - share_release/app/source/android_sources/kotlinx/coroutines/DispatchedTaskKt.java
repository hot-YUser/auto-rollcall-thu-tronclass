...
public final class DispatchedTaskKt {
    ...
    public static final <T> void dispatch(DispatchedTask<? super T> dispatchedTask, int i) {
        ...
        if (!z && (delegate$kotlinx_coroutines_core instanceof DispatchedContinuation) && isCancellableMode(i) == isCancellableMode(dispatchedTask.resumeMode)) {
            ...
            CoroutineContext coroutineContext = delegate$kotlinx_coroutines_core.getContext();
            if (coroutineDispatcher.isDispatchNeeded(coroutineContext)) {
                coroutineDispatcher.mo1885dispatch(coroutineContext, dispatchedTask);
                return;
            } else {
    ...
    public static final <T> void resume(DispatchedTask<? super T> dispatchedTask, Continuation<? super T> continuation, boolean z) {
        ...
        Object objM333constructorimpl = Result.m333constructorimpl(successfulResult$kotlinx_coroutines_core);
        if (z) {
            ...
            if (objUpdateThreadContext != ThreadContextKt.NO_THREAD_ELEMENTS) {
                undispatchedCoroutineUpdateUndispatchedCompletion = CoroutineContextKt.updateUndispatchedCompletion(continuation2, coroutineContext, objUpdateThreadContext);
            } else {
                undispatchedCoroutineUpdateUndispatchedCompletion = null;
            }
            try {
                dispatchedContinuation.continuation.resumeWith(objM333constructorimpl);
        ...
        continuation.resumeWith(objM333constructorimpl);
    }
    ...
    public static final void resumeWithStackTrace(Continuation<?> continuation, Throwable th) {
        ...
        continuation.resumeWith(Result.m333constructorimpl(ResultKt.createFailure(th)));
    }
}
