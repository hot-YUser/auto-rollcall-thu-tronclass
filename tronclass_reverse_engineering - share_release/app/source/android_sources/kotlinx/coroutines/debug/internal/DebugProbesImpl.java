...
public final class DebugProbesImpl {
    ...
    /* high-level source view WARN: Type inference failed for: r1v2, types: [kotlinx.coroutines.debug.internal.DebugProbesImpl$SequenceNumberRefVolatile] */
    static {
    ...
    private final Function1<Boolean, Unit> getDynamicAttach() {
        ...
        if (objNewInstance != null) {
            objM333constructorimpl = Result.m333constructorimpl((Function1) TypeIntrinsics.beforeCheckcastToFunctionOfArity(objNewInstance, 1));
            if (Result.m339isFailureimpl(objM333constructorimpl)) {
                objM333constructorimpl = null;
            }
            return (Function1) objM333constructorimpl;
        }
    ...
    public final String enhanceStackTraceWithThreadDumpAsJson(DebugCoroutineInfo info) {
        ...
        for (StackTraceElement stackTraceElement : listEnhanceStackTraceWithThreadDump) {
            ...
            arrayList.add(StringsKt.trimIndent(sbAppend.append(fileName != null ? toStringWithQuotes(fileName) : null).append(",\n                    \"lineNumber\": ").append(stackTraceElement.getLineNumber()).append("\n                }\n                ").toString()));
        }
    ...
    private final void dumpCoroutinesSynchronized(PrintStream out) {
        ...
        try {
            ...
            for (CoroutineOwner coroutineOwner : SequencesKt.sortedWith(SequencesKt.filter(CollectionsKt.asSequence(debugProbesImpl.getCapturedCoroutines()), new Function1<CoroutineOwner<?>, Boolean>() { // from class: kotlinx.coroutines.debug.internal.DebugProbesImpl$dumpCoroutinesSynchronized$1$2
                ...
                if (Intrinsics.areEqual(debugCoroutineInfoImpl.get_state(), DebugCoroutineInfoImplKt.RUNNING) && listEnhanceStackTraceWithThreadDumpImpl == listLastObservedStackTrace) {
    ...
    private final List<StackTraceElement> enhanceStackTraceWithThreadDumpImpl(String state, Thread thread, List<StackTraceElement> coroutineTrace) {
        ...
        try {
            Result.Companion companion = Result.INSTANCE;
            DebugProbesImpl debugProbesImpl = this;
            objM333constructorimpl = Result.m333constructorimpl(thread.getStackTrace());
        } catch (Throwable th) {
            Result.Companion companion2 = Result.INSTANCE;
            objM333constructorimpl = Result.m333constructorimpl(ResultKt.createFailure(th));
        }
        ...
        StackTraceElement[] stackTraceElementArr = (StackTraceElement[]) objM333constructorimpl;
