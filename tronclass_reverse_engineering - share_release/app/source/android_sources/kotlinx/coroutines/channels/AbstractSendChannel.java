...
public abstract class AbstractSendChannel<E> implements SendChannel<E> {
    ...
    /* high-level source view WARN: Code restructure failed: missing block: B:5:0x000d, code lost:
    ...
    */
    ...
    @Override // kotlinx.coroutines.channels.SendChannel
    public final Object mo1839trySendJP2dKIU(E element) {
        Object objOfferInternal = offerInternal(element);
        if (objOfferInternal == AbstractChannelKt.OFFER_SUCCESS) {
            return ChannelResult.INSTANCE.m1859successJP2dKIU(Unit.INSTANCE);
        }
        if (objOfferInternal == AbstractChannelKt.OFFER_FAILED) {
            Closed<?> closedForSend = getClosedForSend();
            return closedForSend == null ? ChannelResult.INSTANCE.m1858failurePtdJZtk() : ChannelResult.INSTANCE.m1857closedJP2dKIU(helpCloseAndGetSendException(closedForSend));
        }
        if (objOfferInternal instanceof Closed) {
            return ChannelResult.INSTANCE.m1857closedJP2dKIU(helpCloseAndGetSendException((Closed) objOfferInternal));
        }
    ...
    public final void helpCloseAndResumeWithSendException(Continuation<?> continuation, E e, Closed<?> closed) {
        ...
        if (function1 != null && (undeliveredElementExceptionCallUndeliveredElementCatchingException$default = OnUndeliveredElementKt.callUndeliveredElementCatchingException$default(function1, e, null, 2, null)) != null) {
            ...
            continuation.resumeWith(Result.m333constructorimpl(ResultKt.createFailure(undeliveredElementException)));
            return;
        }
        Result.Companion companion2 = Result.INSTANCE;
        continuation.resumeWith(Result.m333constructorimpl(ResultKt.createFailure(sendException)));
    }
    ...
    protected Object enqueueSend(Send send) {
        ...
        do {
            ...
            iTryCondAddNext = prevNode2.tryCondAddNext(send2, lockFreeLinkedListHead2, condAddOp);
    ...
    private final void helpClose(Closed<?> closed) {
        ...
        while (true) {
            ...
            if (receive == null) {
                break;
            } else if (!receive.mo1884remove()) {
                receive.helpRemove();
            } else {
                objM1876constructorimpl$default = InlineList.m1881plusFjFbRPM(objM1876constructorimpl$default, receive);
            }
        }
        if (objM1876constructorimpl$default != null) {
            if (!(objM1876constructorimpl$default instanceof ArrayList)) {
                ((Receive) objM1876constructorimpl$default).resumeReceiveClosed(closed);
            } else if (objM1876constructorimpl$default != null) {
                ArrayList arrayList = (ArrayList) objM1876constructorimpl$default;
     ...
     */
    /* high-level source view WARN: Multi-variable type inference failed */
    /* high-level source view WARN: Type inference failed for: r1v1, types: [kotlinx.coroutines.internal.LockFreeLinkedListNode] */
    ...
    */
    ...
    private final String getQueueDebugStateString() {
        ...
        if (nextNode instanceof Closed) {
            ...
            string = nextNode instanceof Send ? "SendQueued" : "UNEXPECTED:" + nextNode;
        }
        LockFreeLinkedListNode prevNode = this.queue.getPrevNode();
    ...
    /* high-level source view WARN: Removed duplicated region for block: B:25:0x0077  */
    /* high-level source view WARN: Removed duplicated region for block: B:43:0x0069 A[SYNTHETIC] */
    /*
        Code decompiled incorrectly, please refer to instructions dump.
    */
    public final Object sendSuspend(E e, Continuation<? super Unit> continuation) {
        ...
        while (true) {
            if (isFullImpl()) {
                ...
                Object objEnqueueSend = enqueueSend(sendElementWithUndeliveredHandler);
                if (objEnqueueSend == null) {
                ...
                if (objOfferInternal != AbstractChannelKt.OFFER_SUCCESS) {
                    Result.Companion companion = Result.INSTANCE;
                    cancellableContinuationImpl.resumeWith(Result.m333constructorimpl(Unit.INSTANCE));
                    break;
                }
                if (objOfferInternal != AbstractChannelKt.OFFER_FAILED) {
                    if (objOfferInternal instanceof Closed) {
