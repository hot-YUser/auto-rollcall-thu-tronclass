...
from troTHU.radar_solver import GeoPoint, DEFAULT_BOUNDARY_POINTS, LocalFrame, unbounded_grid_offsets
...
class RadarSolverTest(unittest.TestCase):
    def test_enu_round_trip_stays_sub_centimeter_near_thu(self) -> None:
        ...
        for sample in samples:
            ...
            reprojected = frame.to_local(round_tripped)
            self.assertLess(math.hypot(local.x - reprojected.x, local.y - reprojected.y), 0.01)
