...
async function testApis() {
    ...
    let res = await fetch('https://tcidentity.thu.edu.tw/auth/realms/thu/protocol/cas/login?ui_locales=zh-TW&service=https%3A//ilearn.thu.edu.tw/login&locale=zh_TW', { headers: { 'User-Agent': ua } });
    ...
    let finalCookies = [...res.headers.entries()].filter(([key]) => key.toLowerCase() === 'set-cookie').map(([, value]) => value.split(';')[0]).join('; ');

    const endpoints = [
        '/api/radar/rollcalls?api_version=1.1.0',
        '/api/rollcalls/active',
        '/api/rollcall/active',
        '/api/users/me/rollcalls',
        '/api/user/recently-visited-courses'
    ];

    for (const endpoint of endpoints) {
        res = await fetch('https://ilearn.thu.edu.tw' + endpoint, {
            headers: { 'User-Agent': ua, 'Cookie': finalCookies, 'Accept': 'application/json' }
        });
        try {
            console.log(endpoint, ':', res.status, await res.json());
        } catch (error) {
            console.log(endpoint, ':', res.status, 'HTML/Text response');
        }
    }
}
